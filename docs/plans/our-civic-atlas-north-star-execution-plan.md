...
| UCA-017 | Build live civic signals with a non-canonical hot geo index. | First traffic-domain slice now uses GraphQL as the canonical realtime seam with a REST fixture fallback: `useTrafficRealtime`, a refreshable traffic snapshot, a deck.gl speed/volume overlay, and a Traffic island panel while the MDOT RIDE/RunPod/SUMO backends remain unwired. | backend/frontend | Live signals have clear source, mapping, review, visibility, expiry, and promotion rules plus a cautious UI. | GraphQL contract validation, fallback contract review, route smoke, and browser screenshot of the Traffic tab/layer. | Live data becomes rumor amplification or an unbounded ingestion problem. | partial |
...
| UCA-022 | Keep Scene Foundry manifest-driven and offline. OpenUSD is the canonical publication format for finalized scene reconstructions per 2026-05-20 user decision; GLB remains the realtime cache and USD becomes the durable archive. Proto field names must be USD-attribute-compatible (see `docs/design/proto-usd-field-parity-audit.md`). | Existing scene fixtures, `SceneManifest` types, R3F plan, north-star OCA-079 through OCA-081, plus the Anthropic-authored OpenUSD/PairFormer/Nodetree doc adopted on 2026-05-20. | renderer/pipeline | Reviewed manifests can drive USD/GLB/Brush outputs without making public page load depend on live tools or local authoring software. ReconstructionSpec proto fields map mechanically (snake_case to camelCase) onto `civicAtlasSchema` USD attributes. The proto rename PR (XRL-A-003) lands before any USD converter is written. | Contract review, sample asset metadata fixture, validator plan, AND the parity audit at `docs/design/proto-usd-field-parity-audit.md` against `our-civic-atlas-backend/proto/civic_atlas/v1/reconstruction.proto`. | Asset generation bleeds into runtime or loses provenance clarity. Proto-USD field-name drift makes the eventual USD adoption expensive instead of mechanical. | partial |
